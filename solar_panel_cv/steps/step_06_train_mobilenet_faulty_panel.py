"""Train MobileNetV2 on the faulty solar panel dataset."""
...
def train_mobilenet_faulty_panel(ctx: PipelineContext) -> None:
    ...
    model, base = create_mobilenet_model(
        len(class_names),
        training.clip_img_size,
        training.clip_img_size,
    )
    ctx.model = model
    ctx.base_model = base
    ...
    plot_training_history(ctx, history, name="06_mobilenet_training")
    test_loss, test_accuracy = model.evaluate(val_ds, verbose=0)
    print(f"Test accuracy: {test_accuracy:.2f}, loss: {test_loss:.3f}")
    plot_images_with_predictions(ctx, model, val_ds, class_names, name="06_mobilenet_predictions")
